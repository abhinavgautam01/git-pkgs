...
The core commands (`list`, `history`, `blame`, `diff`, `stale`, etc.) work entirely from your git history with no network access. Additional commands fetch external data: `vulns` checks [OSV](https://osv.dev) for known CVEs, `outdated`, `deprecated`, and `licenses` query registries for metadata, and `changelog` fetches upstream changelogs so you can see what changed between versions.
